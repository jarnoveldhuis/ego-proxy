...
//         "https://dl.airtable.com", // Whitelist Airtable's original image domain
//         "https://v5.airtableusercontent.com", // Whitelist Airtable's new image domain
//         "https://www.google-analytics.com" // Whitelist Google Analytics for tracking pixel
//       ],
//       connectSrc: [
//         "'self'",
//         "https://www.googletagmanager.com",
...
app.post("/ask/", async (req, res) => {
  ...
  if (transcript.length > transcriptThreshold && (training || tutorial)) {
    ...
    storyProgress =
      siteId === "meet"
        ? ` The story is a surreal comedy with a twist ending. The orientaion is now ` +
          progress +
          `% complete. Use the transcript thus far to inform what happens next.\n`
        : "";
    const previousProxy = proxies[previousSpeaker] || "";
    console.log("Previous Proxy:", previousProxy);
    let previousProxyProfile;
...
async function summarizeTranscript(transcript, context, user, profile) {
  ...
  "${profile}".`
    : "";
  if (context === "meet") {
    let person = lowerCaseProxies[subdomain].meet ? "You" : user;
...
wss.on("connection", (ws) => {
  ...
  //www.google.com/url?sa=i&url=https%3A%2F%2Fwww.interviewmagazine.com%2Fculture%2Fstavros-halkias-on-ozempic-hunter-biden-and-paneras-charged-lemonade&psig=AOvVaw2Qd4vHrcWVwLGhzBEpSJJf&ust=1726088704197000&source=images&cd=vfe&opi=89978449&ved=0CBQQjRxqFwoTCLCIg9ajuYgDFQAAAAAdAAAAABAE
  // Optionally, send the clientId back to the client
  https: ws.send(JSON.stringify({ type: "clientId", clientId }));
